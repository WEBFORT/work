...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ---------- 10. Точка на пульсирующей линии — двигаем вручную по тем же координатам, что и сама линия ---------- */
  var pulseDot = document.getElementById('pulseDot');
  if (pulseDot && !reduceMotion) {
    var pulsePoints = [[0,23],[470,23],[500,7],[528,39],[556,23],[1200,23]];
    var pulseSegLens = [];
    var pulseTotalLen = 0;
    for (var pi = 0; pi < pulsePoints.length - 1; pi++) {
      var pdx = pulsePoints[pi + 1][0] - pulsePoints[pi][0];
      var pdy = pulsePoints[pi + 1][1] - pulsePoints[pi][1];
      var plen = Math.sqrt(pdx * pdx + pdy * pdy);
      pulseSegLens.push(plen);
      pulseTotalLen += plen;
    }
    var pulseDuration = 4000; // мс — полный проход точки по линии

    var pulseFrame = function (ts) {
      var t = (ts % pulseDuration) / pulseDuration; // 0..1
      var dist = t * pulseTotalLen;
      var acc = 0;
      for (var i = 0; i < pulseSegLens.length; i++) {
        var segLen = pulseSegLens[i];
        if (dist <= acc + segLen || i === pulseSegLens.length - 1) {
          var segT = segLen === 0 ? 0 : (dist - acc) / segLen;
          segT = Math.min(Math.max(segT, 0), 1);
          var x = pulsePoints[i][0] + (pulsePoints[i + 1][0] - pulsePoints[i][0]) * segT;
          var y = pulsePoints[i][1] + (pulsePoints[i + 1][1] - pulsePoints[i][1]) * segT;
          pulseDot.setAttribute('cx', x);
