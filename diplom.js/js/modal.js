export default class Modal {
	...
	onModalClick(event) {
		...
		const modalWindowElement = this.modalWindow; // Элемент, где находится форма

		// Проверяем: клик был внутри окна (формы) ИЛИ клик был на кнопке закрытия
		if (
			target.closest("[data-modal-window]") ||
			target.closest("[data-modal-close]")
		) {
			return; // Если да — ничего не делаем, не закрываем
		}

		// Если клик был ВНЕ окна (на фоне), тогда закрываем
		this.close();
	}
